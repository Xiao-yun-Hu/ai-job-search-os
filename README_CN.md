...
cd ai-job-search-os
bash scripts/install.sh          # 创建 ~/.ai-job-search/、软链 skill、可选装 cron、注册 chrome-devtools MCP

# 每次机器会话启动一次带 CDP 的 Chrome
...
open docs/BROWSER_BACKEND.md   # 浏览器后端（chrome-devtools-mcp + Hermes 前缀）
open docs/RUNTIME_GOVERNOR.md  # Action Governor 策略模型
...
# 8. 跑浏览器驱动任务前，先开启 remote debugging：
open -a "Google Chrome" --args --remote-debugging-port=9222
...
│   ├── BROWSER_BACKEND.md     # v3.1: Hermes + chrome-devtools-mcp 浏览器后端
│   └── RUNTIME_GOVERNOR.md    # v3.1: 运行策略层（意图 vs 执行控制）
├── skills/
│   └── ai-job-search/
│       └── SKILL.md           # 开箱即装的 Hermes skill（带 Step 0 bootstrap）
├── runtime/
│   └── policy.yaml            # v3.1: 每平台 Action Governor 策略
├── scripts/
...
**v3.1（当前）** —— 可安装 Hermes skill + memory bootstrap + 四层执行治理架构 + `chrome-devtools-mcp` 浏览器后端。
- **四层模型**：SKILL.md（意图）→ `runtime/policy.yaml`（策略声明）→ `runtime_guard.py`（执行器级强制拦截）→ `chrome-devtools-mcp`（机械操作）
- **执行器守卫**：`hermes_cli/runtime_guard.py` + `model_tools.py` + `tools/registry.py` 双重拦截点，BOSS `read_only` 模式下所有动作工具在 dispatch 前即被阻断，prompt 层无法绕过
- 7 个单元测试覆盖守卫逻辑
